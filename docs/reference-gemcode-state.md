...
| `.gemcode/fleet_reports.jsonl` | Inbox for completed `org.report` / `job.report` / `agent.report` and formatted **`agent.dm`** / **`agent.broadcast`** lines (drained into the next manager turn when `GEMCODE_FLEET_REPORTS_INJECT=1`) |
